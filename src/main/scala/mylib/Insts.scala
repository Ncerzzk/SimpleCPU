...
object DIVIDER_USE extends Actions
...
object Insts{
  ...
  def RInsts=List(
    ...
    DIV ->(RActions ++ HashMap(
      INST_OP -> OpEnum.ALU,INST_OPSEL->OPArith.DIV,WRITE_REG -> False,
      DIVIDER_USE -> True)),
    DIVU ->(RActions ++ HashMap(
      INST_OP -> OpEnum.ALU,INST_OPSEL->OPArith.DIVU,WRITE_REG -> False,
      DIVIDER_USE -> True
    )),
